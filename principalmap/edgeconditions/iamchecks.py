    ...
    def performChecks(self, session, nodes):
        ...
        for nodeX in tqdm(nodes, ascii=True, desc='Principals Checked'):
            ...
            for nodeY in nodes:
                ...
                elif nodeY.get_type() == 'role' and (
                        nodeY.chk_trust_document(iamclient, nodeX.label) or nodeY.chk_trust_document(iamclient,
                                                                                                     nodeX.get_root_acct_str())):
                    roles.append(nodeY.label)

            # Simulate actions for users
            if len(users) > 0:
                resultlist = testMass(iamclient, nodeX,
                ...
                for action, label, allowed in resultlist:
                    if allowed:
                        ...
                        if action == 'iam:CreateAccessKey':
                            ...
                            result.append(AWSEdge(nodeX, nodeY,
                                                  'IAM_CREATEPASSWORD'))  # TODO: check if target has password to change
            if len(roles) > 0:
