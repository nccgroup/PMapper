...
class CloudFormationChecker():
    ...
    def performChecks(self, session, nodes):
        ...
        for region in tqdm(CloudFormationChecker.regions, ascii=True, desc='CloudFormation Regions Checked'):  # for each region...
                ...
                for stack in fullstacks['Stacks']:
                    ...
                    if 'Outputs' in stack:
                        for output in stack['Outputs']:
                            if accesskeypattern.match(output['OutputValue']) is not None:
                                potentialaccesskeys.append(output['OutputValue'])
                            elif secretkeypattern.match(output['OutputValue']) is not None:
                                potentialsecretkeys.append(output['OutputValue'])
                        for accesskey in potentialaccesskeys:
                            for secretkey in potentialsecretkeys:
                                stsclient = session.create_client('sts', aws_access_key_id=accesskey, aws_secret_access_key=secretkey)
                                try:
